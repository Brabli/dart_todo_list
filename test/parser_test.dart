...
import 'package:dart_todo/enum/option.dart';
import 'package:dart_todo/parser.dart';

void main() {
  group('Parser', () {
    test('Option is null if no option is passed', () {
      Args args = Parser.parse(_createArgs('test'));
    ...
    test('Option is correct if a short flag is passed first', () {
      Args args = Parser.parse(_createArgs('-r test'));
      expect(args.option, Option.remove);
    });

    test('Option is correct if a long flag is passed first', () {
      Args args1 = Parser.parse(_createArgs('--remove test'));
      Args args2 = Parser.parse(_createArgs('--remove=1 test'));
  ...
  group('Args.optionArgs', () {
    test('Option args is null if no arguments are provided', () {
      Args args1 = Parser.parse(_createArgs('-r test'));
      Args args2 = Parser.parse(_createArgs('--remove test'));
    ...
    test('Option args are returned in a list', () {
      Args args1 = Parser.parse(_createArgs('--remove=1 test'));
      Args args2 = Parser.parse(_createArgs('--remove=1,2,3 test'));
      Args args3 = Parser.parse(_createArgs('-r1 test'));
      Args args4 = Parser.parse(_createArgs('-r1,2,3 test'));
    ...
    test('Option args ignore additional equals signs', () {
      Args args = Parser.parse(_createArgs('--remove=1,=2,=3= test'));
      expect(args.optionArgs, ['1', '2', '3']);
    });

    test('Option args does not include empty arguments', () {
      Args args1 = Parser.parse(_createArgs('--remove=,,, test'));
      Args args2 = Parser.parse(_createArgs('--remove=1,,,2 test'));
      Args args3 = Parser.parse(_createArgs('-r,,, test'));
      Args args4 = Parser.parse(_createArgs('-r1,,,2 test'));
      expect(args1.optionArgs, null);
  ...
  group('Args.item', () {
    ...
    test('returns null if no item is found', () {
      Args args1 = Parser.parse(_createArgs('-r1'));
      Args args2 = Parser.parse(_createArgs('--remove'));
      Args args3 = Parser.parse(_createArgs(''));
    ...
    test('is returned when no options are passed', () {
      Args args = Parser.parse(_createArgs(item));
      expect(args.item, item);
    });

    test('is returned when options are passed', () {
      Args args1 = Parser.parse(_createArgs('-r $item'));
      Args args2 = Parser.parse(_createArgs('-r1,2,3 $item'));
      Args args3 = Parser.parse(_createArgs('--remove $item'));
      Args args4 = Parser.parse(_createArgs('--remove=1 $item'));
    ...
    test('capitalises the first letter of an item', () {
      Args args = Parser.parse(_createArgs(item.toLowerCase()));
      expect(args.item, item);
    });
  });
}
